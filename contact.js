...
const goToNextStep = () => {
  ...
  if (element.dataset.step === "reason") {
    const radioButtons = Array.from(element.querySelectorAll(".w-radio"));
    const checkedRadio = radioButtons.find(
      (radio) => !!radio.querySelector(".w--redirected-checked")
    );
    ...
    if (!checkedRadio) {
      ...
      element.querySelectorAll(".w-radio").forEach((radio) => {
